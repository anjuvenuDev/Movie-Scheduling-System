...
class movie_display(tk.Frame):
    ...
    def create_widgets(self):
        self.master.title("NGA Movie Schedule Page")
        ...
        tname = 't' + str(id)
        tinfo = 'tinfo' + str(id)
        cur.execute("SELECT name, type, cast_info FROM {}".format(tname))
        rows1 = cur.fetchall()
        if rows1==() or rows1==[]:
            messagebox.showerror("Error", "No movie exists")
            self.master.show_main_window(self.username)
        cur.execute("select  avail_screens, ts1_wday, ts2_wday, ts3_wday, ts1_wend, ts2_wend, ts3_wend, ts4_wend, ts5_wend from {}".format(tinfo))    
        rows2 =cur.fetchall()
        rows=()
    ...
    def screen_calc(self):
        ...
        # Header Label
        header_label = tk.Label(self, text="YOUR PERSONALISED MOVIE SCHEDULE", font=("Century Gothic", 24, "bold"), pady=20, fg="black")
        header_label.pack()
        ...
        tot_demand=0
        dlist=[]
        q2 = 'select name, unbooked_seats_day, unbooked_seats_end, demand_score, cast_weightage from {}'.format(tname)
        cur.execute(q2)
        rows = cur.fetchall()
        print(rows)
        for i in rows:
            ...
            if booking_rate_end >= 90:
                avail_screens -= 1
                cur.execute('replace into {}(name, avail_screens) values(%s, %s)'.format(tinfo), (i[0], avail_screens))
                con.commit()
                ...
                q1 = 'delete from {} where name=%s'.format(tname)
                cur.execute(q1, (i[0],))
                con.commit()
        ...
        for i in minfo:
            ...
            if allot_screens==0:
                ...
                cur.execute(q1, (i[0],))
                con.commit()
                q1= 'delete from {} where name=%s'.format(tinfo)
                cur.execute(q1, (i[0],))
                con.commit()
                print("DELETEDDDD")
            else:
                cur.execute('replace into {}(name, avail_screens) values(%s, %s)'.format(tinfo), (i[0], allot_screens))
                con.commit()
        ...
        for i in rows:
            if count==2:
                break
            if i[1] in [max1, max2]:
                cur.execute('update {} set avail_screens = %s where name = %s'.format(tinfo), (i[1]+1, i[0]))
                con.commit()
                count+=1
        cur.execute(q1)
        rows=cur.fetchall()
        for i in rows:
            if i[1]>=12:
                cur.execute('update {} set ts1_wday = %s, ts2_wday = %s, ts3_wday = %s where name = %s'.format(tinfo), (hp_time_slot_day[0], hp_time_slot_day[1], hp_time_slot_day[2], i[0]))
                con.commit()
                cur.execute('update {} set ts1_wend = %s, ts2_wend = %s, ts3_wend = %s, ts4_wend = %s, ts5_wend = %s where name = %s'.format(tinfo), (hp_time_slot_end[0], hp_time_slot_end[1], hp_time_slot_end[2], hp_time_slot_end[3], hp_time_slot_end[4], i[0]))
                con.commit()
            elif 6<i[1]<=12:
                cur.execute('update {} set ts1_wday = %s, ts2_wday = %s, ts3_wday = %s where name = %s'.format(tinfo), (lp1_time_slot_day[0], lp1_time_slot_day[1], lp1_time_slot_day[2], i[0]))
                con.commit()
                cur.execute('update {} set ts1_wend = %s, ts2_wend = %s, ts3_wend = %s, ts4_wend = %s, ts5_wend = %s where name = %s'.format(tinfo), (lp_time_slot_end[0], lp_time_slot_end[1], lp_time_slot_end[2], lp_time_slot_end[3], lp_time_slot_end[4], i[0]))
                con.commit()
            else:
                cur.execute('update {} set ts1_wday = %s, ts2_wday = %s, ts3_wday = %s where name = %s'.format(tinfo), (lp2_time_slot_day[0], lp2_time_slot_day[1], lp2_time_slot_day[2], i[0]))
                con.commit()
                cur.execute('update {} set ts1_wend = %s, ts2_wend = %s, ts3_wend = %s, ts4_wend = %s, ts5_wend = %s where name = %s'.format(tinfo), (lp_time_slot_end[0], lp_time_slot_end[1], lp_time_slot_end[2], lp_time_slot_end[3], lp_time_slot_end[4], i[0]))
                con.commit()
    
        disp_button = tk.Button(self, text="SHOW SCHEDULE", command=self.create_widgets, font=("Helvetica", 14), bg="#4CAF50", fg="white")
        disp_button.pack(pady=10)  

        rec_button = tk.Button(self, text="RECOMMENDED SCHEDULE", command=self.prev, font=("Helvetica", 14), bg="#4CAF50", fg="white")
    ...
    def prev(self):
        self.master.show_rec_movie(self.username)
